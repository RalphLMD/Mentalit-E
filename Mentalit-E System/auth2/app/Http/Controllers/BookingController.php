...
class BookingController extends Controller
{
    ...
    public function store_quick_booking(Request $request, $service) { 
        ...
        $var2 = $vars->serviceLength;

        $totalDuration =  $start->addHours($var2)->format('Y-m-d H:i:s');
    ...
    public function display_booking_clinic (Booking $bookings, Clinic $clinic) { 
        ...
        return view ('/clinic/booking/index', ['clinic'=>$clinic]);
    }    
    ...
    public function getBooking(Request $request, Booking $booking) {
        ...
        if ($request->ajax()) {
            $data = Booking::where('clinic_id', $var->clinic_id)->with(['userBooking', 'serviceBooking', 'employeeBooking']);
    ...
    public function update_link(Request $request, $id) {
        ...
        return redirect()->back()->with('message', 'Link has been updated!');
    } 
    ...
    public function cancel_booking(Clinic $clinic, $id) {  
        $var = Booking::find($id); 
        $clinic = Clinic::where('id', $var->clinic_id)->get()->first();
        $user = User::where('id', $var->user_id)->get()->first();
        ...
        ClinicNotification::create([
            ...
            'notifDescription' => 'The appointment for' . $user->email . 'has been cancelled',
            'notifDateTime' => now(),
        ]);

        Notification::create([
            'user_id' => $user->id,
            'notifDescription' => 'Your appointment at' . $clinic->clinicName . 'has been cancelled',
            'notifDateTime' => now(),
        ]);
        ...
        return redirect()->back()->with('message', 'Booking has been declined and the user will be notified');
    }
    ...
    public function update_first_booking(Request $request, $id, Employee $employee) { //assinging of consultation (step 2) - In Progress
        ...
        return redirect()->back()->with('message', 'Booking has been assigned to the employee');
    }
    ...
    public function store_booking_remarks(Request $request, $id) {
        ...
        return redirect()->back()->with('message', 'Remarks has been submitted!');
    } 
    ...
    public function display_booking_patient(Booking $bookings) { //display all booking of the patient
        $user = auth()->user();
        return view ('/booking/patient.index', ['user'=>$user]);
    }
    ...
    public function getBookingPatient(Request $request, Booking $booking) {
        ...
        if ($request->ajax()) {
            $data = Booking::where('user_id', $user->id)->with([ 'clinicBooking' ,'serviceBooking']);
            return Datatables::eloquent($data)
                ->addIndexColumn()  
    ...
    public function store_booking_ratings(Request $request, $id) {
        ...
        return redirect()->back()->with('message', 'Ratings has been submitted!');
    } 
    ...
    public function cancel_booking_patient(Request $request, $id) {
        ...
        return redirect()->back()->with('message', 'File for Cancellation has been submitted!');
    }     
