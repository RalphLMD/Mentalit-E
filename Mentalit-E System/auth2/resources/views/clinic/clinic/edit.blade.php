            ...
            <div class="shadow p-5 w-100 standard">
                <div class="text-center">		
                    @if ($clinic->clinicMainPhoto == null)  
                        <img src="{{ asset('storage/avatars/hospital.png')}}" width=150>		
                    @else
                        <img src="{{ asset('storage/avatars/'.$clinic->clinicMainPhoto) }}" class="profile" width=150>
                    @endif				
                    <form action="/clinic/clinic/{{$clinic->id}}/picture" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <input class="form-control w-25 m-auto mt-3" type="file" name="clinicMainPhoto" accept="image/*" required autocomplete="clinicMainPhoto"> 
                    ...
                    <div class="col-lg-6">
                        <div class="mb-3 w-75 auto">
                            <form action="/clinic/clinic/{{$clinic->id}}/subscription" method="post" enctype="multipart/form-data">
                                ...
                                @enderror   
                                <input class="form-control m-auto mt-3" type="file" name="paymentProof" accept="image/*, application/pdf, application/msword" required autocomplete="paymentProof">
                                @error('paymentProof')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
